// clients/admin-web-panel/src/app/login/page.tsx
'use client';
...
export default function LoginPage({ onLoginSuccess }: LoginPageProps) {
  ...
  const { currentUser, loading, signIn, registerUser } = useAuth(); // Destructure registerUser from useAuth hook
  const router = useRouter();

  // Redirect if the user is already authenticated
  useEffect(() => {
    if (!loading && currentUser) {
      router.push('/dashboard'); // *** CHANGED: Redirect to /dashboard instead of / ***
    }
  ...
  const handleRegister = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!email || !password) {
      showMessageModal('Please enter both email and password for registration.', 'error');
      return;
    }

    try {
      await registerUser(email, password); // Use the registerUser function
  ...
  if (loading || currentUser) {
    return <div className="min-h-screen flex items-center justify-center bg-beaverNeutral-light">Loading...</div>;
  }
    ...
    <div className="min-h-screen flex items-center justify-center bg-beaverNeutral-light p-4">
      <div className="bg-white p-8 rounded-lg shadow-xl w-full max-w-md">
        <h2 className="text-3xl font-bold text-beaverBlue-dark text-center mb-6">
          {showRegisterForm ? 'Register for BeaverOS Admin' : 'Login to BeaverOS Admin'}
        </h2>
